...
default_cfgs = {
    # patch models (weights from official Google JAX impl) pretrained on ImageNet-21k finetuned on ImageNet-1k
    ...
    # How to train your ViT (augreg) weights trained on ImageNet-1k only
    'vit_small_patch16_224_in1k': _cfg(
        url='https://storage.googleapis.com/vit_models/augreg/'
        'S_16-i1k-300ep-lr_0.001-aug_medium2-wd_0.1-do_0.0-sd_0.0--imagenet2012-steps_20k-lr_0.01-res_224.npz'),
    'vit_small_patch16_384_in1k': _cfg(
        url='https://storage.googleapis.com/vit_models/augreg/'
        'S_16-i1k-300ep-lr_0.001-aug_medium2-wd_0.1-do_0.0-sd_0.0--imagenet2012-steps_20k-lr_0.01-res_384.npz',
        input_size=(3, 384, 384), crop_pct=1.0),
    'vit_base_patch32_224_in1k': _cfg(
        url='https://storage.googleapis.com/vit_models/augreg/'
        'B_32-i1k-300ep-lr_0.001-aug_medium2-wd_0.1-do_0.1-sd_0.1--imagenet2012-steps_20k-lr_0.01-res_224.npz'),
    'vit_base_patch32_384_in1k': _cfg(
        url='https://storage.googleapis.com/vit_models/augreg/'
        'B_32-i1k-300ep-lr_0.001-aug_medium2-wd_0.1-do_0.1-sd_0.1--imagenet2012-steps_20k-lr_0.01-res_384.npz',
        input_size=(3, 384, 384), crop_pct=1.0),
    ...
    'vit_base_patch16_384_in1k': _cfg(
        url='https://storage.googleapis.com/vit_models/augreg/'
        'B_16-i1k-300ep-lr_0.001-aug_strong2-wd_0.1-do_0.1-sd_0.1--imagenet2012-steps_20k-lr_0.01-res_384.npz',
        input_size=(3, 384, 384), crop_pct=1.0),
    ...
    # SAM trained models (https://arxiv.org/abs/2106.01548) trained on ImageNet-1k only
    'vit_base_patch32_224_sam_in1k': _cfg(
        url='https://storage.googleapis.com/vit_models/sam/ViT-B_32.npz'),
    'vit_base_patch16_224_sam_in1k': _cfg(
...
class LoRA(nn.Module):
    def __init__(self, dim, r, init_type='shared'):
        super().__init__()
...
class Attention_LoRA(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, scale=None, attn_drop=0., proj_drop=0., r=64, n_tasks=10,
                 is_shared=False, is_specific=False, msa=[0,0,0]):
        super().__init__()
        ...
        # shared LoRA
        if is_shared:
            self.lora_q = LoRA(dim, r, 'shared') if msa[0] else None
            self.lora_k = LoRA(dim, r, 'shared') if msa[1] else None
            self.lora_v = LoRA(dim, r, 'shared') if msa[2] else None
            # memory of shared LoRA
            self.lora_q_old = LoRA(dim, r, 'zero') if msa[0] else None
            self.lora_k_old = LoRA(dim, r, 'zero') if msa[1] else None
            self.lora_v_old = LoRA(dim, r, 'zero') if msa[2] else None
        
        # specific LoRA
        if is_specific:
            self.lora_q = nn.ModuleList([LoRA(dim, r, 'specific') for _ in range(n_tasks)]) if msa[0] else None
            self.lora_k = nn.ModuleList([LoRA(dim, r, 'specific') for _ in range(n_tasks)]) if msa[1] else None
            self.lora_v = nn.ModuleList([LoRA(dim, r, 'specific') for _ in range(n_tasks)]) if msa[2] else None

        self.rank = r
    
    def init_param(self):
        def _init_lora(module):
            if module is not None:
                module.init_parameters()

        if self.is_shared:
            _init_lora(self.lora_q)
            _init_lora(self.lora_k)
            _init_lora(self.lora_v)
            _init_lora(self.lora_q_old)
            _init_lora(self.lora_k_old)
            _init_lora(self.lora_v_old)
        
        if self.is_specific:
            for i in range(len(self.lora_q)) if self.msa[0] else []:
                _init_lora(self.lora_q[i])
            for i in range(len(self.lora_k)) if self.msa[1] else []:
                _init_lora(self.lora_k[i])
            for i in range(len(self.lora_v)) if self.msa[2] else []:
                _init_lora(self.lora_v[i])
    
    def save_old_shared_lora(self):
        if not self.is_shared:
            return
        with torch.no_grad():
            self.lora_q_old.load_state_dict(self.lora_q.state_dict()) if self.msa[0] else None
            self.lora_k_old.load_state_dict(self.lora_k.state_dict()) if self.msa[1] else None
            self.lora_v_old.load_state_dict(self.lora_v.state_dict()) if self.msa[2] else None

    def forward(self, x, task_id, blk_weight=None, use_new=False):
        B, N, C = x.shape
        qkv = self.qkv(x)
        ...
        if self.is_shared:
            if not use_new:
                q = q + self.lora_q_old(x) if self.msa[0] else q
                k = k + self.lora_k_old(x) if self.msa[1] else k
                v = v + self.lora_v_old(x) if self.msa[2] else v
            else:
                q = q + self.lora_q(x) if self.msa[0] else q
                k = k + self.lora_k(x) if self.msa[1] else k
                v = v + self.lora_v(x) if self.msa[2] else v
            
        if self.is_specific:
            if blk_weight is None:
                blk_weight = torch.ones(3, device=x.device)
            
            q = q + self.lora_q[task_id](x) * blk_weight[0] if self.msa[0] else q
            k = k + self.lora_k[task_id](x) * blk_weight[1] if self.msa[1] else k
            v = v + self.lora_v[task_id](x) * blk_weight[2] if self.msa[2] else v
...
class Block(nn.Module):
    ...
    def __init__(
            self, blk_id, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0., init_values=None, 
            drop_path=0., norm_layer=nn.LayerNorm, act_layer=nn.GELU, n_tasks=10, rank=64, 
            shared_pos=None, specific_pos=None, msa=[0,0,0]):
        super().__init__()

        self.is_shared = blk_id in shared_pos
        self.is_specific = blk_id in specific_pos
        
        self.norm1 = norm_layer(dim)
        self.attn = Attention_LoRA(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, n_tasks=n_tasks, r=rank,
                                   is_shared=self.is_shared, is_specific=self.is_specific, msa=msa)
    ...
    def forward(self, x, task_id=None, blk_weight=None, use_new=False):
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x), task_id, blk_weight, use_new)))
...
class VisionTransformer(nn.Module):
    """ Vision Transformer for continual learning
    ...
    def __init__(
        ...
        """
        Args:
            msa (list): which of Q, K, V to apply LoRA, e.g., [1,0,1] means Q and V
            shared_pos (list): for task-shared LoRA
            specific_pos (list): for task-specific LoRA
        """
        super().__init__()
        ...
        self.blocks = nn.Sequential(*[
            block_fn(
                blk_id=i, dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate, attn_drop=attn_drop_rate, 
                init_values=init_values, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer, n_tasks=n_tasks, rank=rank,
                shared_pos=self.shared_pos, specific_pos=self.specific_pos, msa=self.msa)
