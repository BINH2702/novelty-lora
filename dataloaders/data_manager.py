...
from dataloaders.data import iCIFAR100, iIMAGENET_R, iIMAGENET_A, iCUB, iDomainNet, iCIFAR10
...
class DataManager(object):
    def __init__(self, dataset_name, shuffle, seed, init_cls, increment, args=None):
        ...
        offset = len(self._class_order) - sum(self._increments) # last increment
        if offset > 0:
    ...
    def get_task_size(self, task):
        return self._increments[task]  # number of classes in task
    ...
    def _setup_data(self, dataset_name, shuffle, seed):
        ...
        logging.info("Class index order: %s", summarize_class_indices(sorted(self._class_order), 5, 5))
...
def summarize_class_indices(class_indices, head=5, tail=5):
    total = len(class_indices)
    if total <= head + tail:
        return ', '.join(str(i) for i in class_indices)
    head_part = ', '.join(str(i) for i in class_indices[:head])
    tail_part = ', '.join(str(i) for i in class_indices[-tail:])
